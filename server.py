...
H = 10
j_val = -1
alpha_exp = Expression('0.5*j_val*(tanh(x[1] - (mid - (H/2))) - tanh(x[1] - (mid + (H/2))))', element = FA.ufl_element(), j_val = j_val, mid = width/2, H = H)
alpha = interpolate(alpha_exp, FA)
...
def interpolate_up(original_grid):
    ...
    x_original, y_original = np.meshgrid(np.linspace(0, 1, nx + 1), np.linspace(0, 1, ny + 1))
    x_new, y_new = np.meshgrid(np.linspace(0, 1, 50), np.linspace(0, 1, 50))
    ...
    interpolated_grid = values_new.reshape((50, 50))
...
def time_step(arr):
        ...
        for dof in dofs:                                                            
            if not dof in visited:
                ...
                if my_first <= global_dof < my_last:                  
                    ...
                    px, py = int(0.5*dof_coord[0]), int(0.5*dof_coord[1])
    ...
    ux_arr = np.zeros([nx + 1, ny + 1])
    uy_arr = np.zeros([nx + 1, ny + 1])
    #Qxx_arr = np.zeros([26, 26])
    #Qxy_arr = np.zeros([26, 26])

    for dof in range(len(dof_x)):

        px, py = int(0.5*dof_y[dof]), int(0.5*dof_x[dof])
    ...
    obs = np.stack((ux_arr, uy_arr, alpha_arr), axis=0)
